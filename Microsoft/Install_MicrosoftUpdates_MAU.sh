...
# Set workflow type (ex: Policy or Self Service)
workFlowType=$4
...
function DetermineLoginState() {
	...
	if [ "$CONSOLE" == "" ]; then
    	...
    	hostName=$(hostname)
    	fullName=$(finger | grep "$CONSOLE" | sed -n 1p | awk '{print $2,$3}')
    	echo "Script result: User $CONSOLE ($fullName)"
		echo "Script result: Logged onto $hostName"
    	userID=$(/usr/bin/id -u "$CONSOLE")
		CMD_PREFIX="/bin/launchctl asuser $userID "
	fi
}
