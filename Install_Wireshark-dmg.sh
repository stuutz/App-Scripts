...
# DESCRIPTION
#
#	This script installs the latest version of Wireshark.
#
#	File type: dmg
#
# VERSION
...
##########################################################################################
AppName="Wireshark"
latestVersion=$(/usr/bin/curl -sL 'https://www.wireshark.org/#download' | grep "stable" | tr -d 'The current stable release of Wireshark is ' | sed 's/.$//')
...
if [ "$arch" == "arm64" ]; then
	...
	DownloadLink="https://2.na.dl.wireshark.org/osx/Wireshark%20${latestVersion}%20Arm%2064.dmg"
          
elif [ "$arch" == "i386" ]; then
    
    echo "Intel Found"
    
    DownloadLink="https://2.na.dl.wireshark.org/osx/Wireshark%20${latestVersion}%20Intel%2064.dmg"
