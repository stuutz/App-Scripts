...
# DESCRIPTION
#
#	This script installs the latest version of kdiff3.
#
#	File type: dmg
#
# VERSION
...
##########################################################################################
AppName="kdiff3"
latestVersion=$(/usr/bin/curl -sL 'http://kdiff3.sourceforge.net' | grep "Version" | sed -n 1p | awk '{print $3}' | tr -d '</dt>')
...
if [ "$arch" == "arm64" ]; then
	...
	DownloadLink="https://cfhcable.dl.sourceforge.net/project/${AppName}/${AppName}/${latestVersion}/${AppName}-${latestVersion}-MacOSX-64Bit.dmg"
